...
void pktDaemon() {
  ...
  if(millis() - pdt >= 1) { 	// at least every 1 ms
	...
	if (NODETYPE == SLAVE) {
	  ...
	  for(uchar i=4; i<rxbuff[0]; i++) {
	    sendByte(rxbuff[i]);	// relay content of the pkt
	  }
	  #if DEBUGMODUS
	  Serial.print("RLY DATA");
	  printPkt();
	  #endif
	}
	...
	else if(rxbuff[5] == timestamp){	// the ANID is for me!
