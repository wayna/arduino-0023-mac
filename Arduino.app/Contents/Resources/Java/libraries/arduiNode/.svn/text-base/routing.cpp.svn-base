...
void mangageNextNodes(uchar id, uchar htm) {
  ...
  do {
    swapped = false;
    for(uchar i = 0; i < NODE_TABLE_SIZE-1; i++) {
      for(uchar j = i+1; j < NODE_TABLE_SIZE-1; j++) {
...
void printNodes() { 
  Serial.println();
  for(uchar i = 0; i < NODE_TABLE_SIZE; i++) {
    Serial.print(" id: ");
...
uchar allocateID() {
  static uchar IDcnt = 1;	// how many ID's are out there
  return ++IDcnt;
}
