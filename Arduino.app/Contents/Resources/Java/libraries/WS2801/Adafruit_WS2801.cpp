...
Adafruit_WS2801 ws2801((uint16_t) 170, (uint8_t) 4, (uint8_t) 5, (uint8_t) 0); // ojo añadido pixel node
...
void Adafruit_WS2801::show(void) {
    ...
    if(hardwareSPI) {
        ...
        for(i=0; i<nl3-2; i+=3 ) { //data bits
            *dataport |=  datapinmask; //includes 1 bit as start “1” plus 3 groups 5 bits grey level.
            
            *clkport |=  clkpinmask;
            *clkport &= ~clkpinmask;
            
            for(bit=0x80; bit<=4; bit >>= 1) { //only 5 bits from 8
                if(pixels[i] & bit)
                    *dataport |=  datapinmask;
                else
                    *dataport &= ~datapinmask;
